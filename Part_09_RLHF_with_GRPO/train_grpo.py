...
from policy import PolicyWithoutValue  # we will ignore the value head
...
sys.path.append(str(_P(__file__).resolve().parents[1]/'Part_07_Reward_Modeling'))
...
def main():
    ...
    policy = PolicyWithoutValue(vocab_size, block_size, n_layer, n_head, n_embd).to(device)
    policy.lm.load_state_dict(ckpt['model'])
    policy.eval()
    
    ref = PolicyWithoutValue(vocab_size, block_size, n_layer, n_head, n_embd).to(device)
    ...
    while step < args.steps:
        ...
        policy.train()
        logits_new, _ = policy(seq, None)  # ignore value head

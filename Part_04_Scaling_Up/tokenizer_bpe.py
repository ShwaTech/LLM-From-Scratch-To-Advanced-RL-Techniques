...
In LLMs, BPE stands for Byte Pair Encoding.

🔍 Meaning:
    Byte Pair Encoding is a subword tokenization algorithm that:
    Starts from individual characters (bytes),
    Repeatedly merges the most frequent pair of tokens (byte sequences),
    Builds a vocabulary of common subwords.

💡 Why it's used in LLMs:
    Handles rare words and new tokens efficiently.
    Balances between character-level and word-level tokenization.
    Keeps the vocabulary compact while still representing most text naturally.

🧠 Example:
    "lower" → ["low", "er"]
    "lowest" → ["low", "est"]
Both share the subword “low”, making it easier for the model to generalize.
...
class BPETokenizer:
    ...
    def __init__(self, vocab_size: int = 32000, special_tokens: List[str] | None = None):
        if ByteLevelBPETokenizer is None:
            raise ImportError("Please `uv add tokenizers` for BPETokenizer.")

...
A Reward Model (RM) is a neural network that learns to judge how good a model’s output is — essentially, it assigns a score (a reward) to text generated by a language model.
You can think of it as the “critic” in a teacher–student setup:
    1. The LLM (policy model) generates possible answers.
    2. The reward model evaluates them — giving higher scores to better, more human-like responses.
...
Then we train using a pairwise loss, such as:
    🧮 Bradley–Terry Loss:
        𝐿 = −log(σ(r_pos − r_neg))
This means:
    If r_pos > r_neg, loss is small ✅
    If r_pos < r_neg, loss is large ❌
The model thus learns to give higher scores to human-preferred responses.

📈 Why It’s Important

✅ Aligns the model with human values and preferences
✅ Enables safe and polite LLM behavior
